...
class Chips:
    ...
    def bet(self, amount):
        if amount > self.total:
            print(f'Unable to draw any chips. {self.name} you are out of funds.')
...
class Player(Chips):
    ...
    def hand_value(self):
        ...
        card1 = values[self.player_hand[0][1]] in royalty_list
        card2 = values[self.player_hand[1][1]] in royalty_list
        card3 = values[self.player_hand[2][1]] in royalty_list

        # Ace check
        if self.player_hand[0][1] == self.player_hand[1][1] == self.player_hand[2][1]:
            player_hand_value += 400 + 3 * values[self.player_hand[0][1]]
            return player_hand_value

        # Royalty check
        elif card1 and card2 and card3:
            player_hand_value += 300
...
def game_input(name, already_bet, current_player, other_player, chips_board):
    move = 'not bet'
    while move.lower() not in ['bet', 'show', 'close']:
        ...
        if move.lower() == 'bet':
            ...
            current_player.show_cards()
            other_player.show_cards()
            other_player.receive(game_board.game_win())
...
def take_extra_cards(current_name, other_name, current_player, deck, board):
    ...
    if player1_extra_cards.lower() == 'b':
...
def game_round(entity1, entity1_name, entity2, entity2_name, board, deck):
    ...
    already_bet = 1
    while round_state:
        round_state = game_input(entity1_name, already_bet, entity1, entity2, board)
        if round_state is False:
            break
        else:
            already_bet = round_state

        round_state = game_input(entity2_name, already_bet, entity2, entity1, board)
        if round_state is False:
            break
        else:
            already_bet = round_state
...
while continue_game:
    round_count += 1
    print(f'{round_count} has started')
